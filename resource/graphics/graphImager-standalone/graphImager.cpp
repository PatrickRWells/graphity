...
int main() {
    ...
    for(int k = 0; k < numGraphs; k++) {
        std::cout << "Generating image of graph " << k+1 << " of " << numGraphs << std::endl;
        int imageSize;
        ...
        int graphSize = graphs[k]->getSize();
        /*
        ...
        while(sweepCount < numSweeps) {
            double energyOld = energyVal(locations, *graphs[k]);
            ...
            double energyNew = energyVal(locations, *graphs[k]);
